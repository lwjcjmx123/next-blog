  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && 'PerformanceObserver' in window) {
          ...
          if (metricName) {
            ...
            setMetrics(prev => {
              ...
              if (existing) {
                ...
                return [...prev, { name: metricName, value, rating, description }];
              }
            });
          }
        });
      ...
      try {
        ...
        const typesToObserve = ['largest-contentful-paint', 'first-input', 'layout-shift']
          .filter(type => supportedTypes.includes(type));
        
        if (typesToObserve.length > 0) {
          vitalsObserver.observe({ entryTypes: typesToObserve });
          addLog(`Web Vitals Observer 已启动，监听: ${typesToObserve.join(', ')}`);
        } else {
      ...
      if (navigation) {
        ...
        const newMetrics: PerformanceMetric[] = [
          ...
          },
          {
            name: 'DOM Load',
            value: domLoadTime,
            rating: domLoadTime <= 1500 ? 'good' : domLoadTime <= 3000 ? 'needs-improvement' : 'poor',
            description: 'DOM加载时间'
          },
          {
            name: 'Page Load',
        ...
        setMetrics(prev => {
          const filtered = prev.filter(m => !['TTFB', 'DOM Load', 'Page Load'].includes(m.name));
                  ...
                  <tbody>
                    {paintMetrics.map((metric, index) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className="py-2">{metric.startTime.toFixed(2)}ms</td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              <div>
                ...
                <ul className="space-y-1 text-gray-600 dark:text-gray-400">
                  <li><strong>LCP</strong>: 最大内容绘制 (≤2.5s 优秀)</li>
                  <li><strong>FID</strong>: 首次输入延迟 (≤100ms 优秀)</li>
                  <li><strong>CLS</strong>: 累积布局偏移 (≤0.1 优秀)</li>
                </ul>
              </div>
              <div>
                <h3 className="font-semibold mb-2">其他指标</h3>
                <ul className="space-y-1 text-gray-600 dark:text-gray-400">
                  <li><strong>FCP</strong>: 首次内容绘制 (≤1.8s 优秀)</li>
                  <li><strong>TTFB</strong>: 首字节时间 (≤200ms 优秀)</li>
                  <li><strong>DOM Load</strong>: DOM加载时间</li>
                </ul>
              </div>
            </div>
          </Card>
