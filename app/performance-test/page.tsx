...
export default function PerformanceTestPage() {
  ...
  useEffect(() => {
    ...
    const handlePerformanceMetric = (event: CustomEvent<PerformanceMetric>) => {
      ...
      setLogs(prev => {
        const newLog = `${new Date().toLocaleTimeString()}: ${event.detail.name} = ${event.detail.value.toFixed(2)}ms (${event.detail.rating})`;
        // 避免重复添加相同的日志
    ...
    // 添加初始日志
    setLogs(prev => [...prev, `${new Date().toLocaleTimeString()}: 页面加载，开始监听性能指标...`]);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* 性能监控组件 */}
      <PerformanceMonitor config={{ enableConsoleLog: true, enableDashboard: true }} />
